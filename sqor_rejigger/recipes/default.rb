...
execute "service_stop" do
    cwd node[:sqor_rejigger][:base_dir]
    command "rel/rejigger/bin/rejigger stop"
    only_if do FileTest.directory?(node[:sqor_rejigger][:rel_dir]) end
end

# Create base dir
directory node[:sqor_rejigger][:base_dir] do
    owner "root"
    group "root"
    action :create
end
...
# Pull from github
git node[:sqor_rejigger][:base_dir] do
    repository node[:sqor_rejigger][:git][:url]
    reference node[:sqor_rejigger][:git][:deploy_tag]
    action :sync
end
...
execute "service_start" do
    cwd node[:sqor_rejigger][:base_dir]
    command "rel/rejigger/bin/rejigger start"
    action :run
end
