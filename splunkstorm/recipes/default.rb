...
template "#{node['splunkstorm']['forwarder_home']}/#{node[:splunkstorm][:license_name]}" do
  source "#{node[:splunkstorm][:license_name]}.erb"
  mode 0644
  owner "root"
  group "root"
end
...
execute "#{splunk_cmd} install app #{node['splunkstorm']['forwarder_home']}/#{node[:splunkstorm][:license_name]} -auth #{node['splunkstorm']['auth']} && echo true > /opt/splunk_setup_license" do
  not_if do
